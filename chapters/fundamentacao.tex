...
Este capítulo estabelece a base conceitual para o presente trabalho. A discussão inicia-se, na \autoref{sec:fund-conceito-privacidade}, com a evolução histórica do conceito de privacidade, desde suas primeiras formulações até os desafios impostos pela era digital. Em seguida, o foco se volta para o arcabouço legal brasileiro, detalhando na \autoref{sec:fund-lgpd} os principais artigos e princípios da Lei Geral de Proteção de Dados Pessoais (LGPD) que regem o tratamento de dados, especialmente no contexto da saúde e da pesquisa científica. Posteriormente, a \autoref{sec:fund-valor-dados} apresenta o valor e a estrutura dos dados clínicos, justificando sua importância para a inovação. Com esse alicerce, o capítulo explora, na \autoref{sec:fund-metodos-anon}, os principais métodos e técnicas de anonimização descritos na literatura e, por fim, aborda na \autoref{sec:fund-metricas} as métricas de avaliação utilizadas para medir o balanço fundamental entre a proteção da privacidade e a utilidade dos dados anonimizados.
% ---
\section{A Evolução do Conceito de Privacidade}
\label{sec:fund-conceito-privacidade}

A discussão sobre privacidade é multifacetada e evoluiu significativamente com o avanço da tecnologia e das dinâmicas sociais. Para compreender os desafios atuais da proteção de dados na saúde, é essencial revisitar os conceitos fundamentais que moldaram o debate e que servem de alicerce para a legislação contemporânea.

\subsection{O Direito de Ser Deixado em Paz (Warren e Brandeis)}
\label{subsec:fund-warren-brandeis}

Em uma das primeiras definições de privacidade, Samuel Warren e Louis Brandeis \cite{WarrenBrandeis1890} articularam a existência de um "direito a ser deixado em paz" (\textit{the right to be let alone}). O argumento dos autores é de que a proteção de pensamentos, sentimentos e emoções expressos por meio da escrita ou das artes contra plágio ou apropriação física não era um princípio da propriedade privada (sob o qual a propriedade intelectual é baseada), mas sim um direito a "inviolabilidade pessoal".

Portanto, não somente as produções intelectuais ou artísticas, mas também as expressões casuais do dia a dia — pensamentos, emoções e ações - deveriam ser protegidas da exposição pública indesejada. Com isso, a privacidade foi estabelecida como um direito universal que não deriva de um contrato ou de uma relação de propriedade, mas sim como um princípio inerente à dignidade humana, lançando as bases para todo o debate futuro sobre o tema.

\subsection{Autodeterminação Informativa e o Controle de Dados (Westin)}
\label{subsec:fund-westin}

Em 1967, já no início da era da computação, em \textit{Privacy and Freedom} \cite{Westin1967}, Alan Westin analisou como as novas tecnologias de vigilância e a capacidade de armazenamento e processamento de dados em computadores, criavam uma ameaça inédita à liberdade individual. Diante desse cenário, Westin formulou sua influente definição de privacidade como o direito de um indivíduo de determinar quando, como e em que medida as informações sobre si são comunicadas a terceiros \cite{Westin1967}. 

Essa noção, conhecida como "autodeterminação informativa", deslocou o foco de um direito passivo ao isolamento para um poder ativo de controle sobre o fluxo de dados pessoais.

\subsection{A Integridade Contextual (Nissenbaum)}
\label{subsec:fund-nissenbaum}
...
A violação da privacidade ocorre quando essas normas são quebradas. \citeonline{Nissenbaum2009} modela essas normas a partir de três parâmetros: os atores (quem envia, quem recebe), os atributos (o tipo de informação) e os princípios de transmissão (as regras que governam o fluxo). O contexto da saúde pode ser um exemplo: um paciente (ator) espera que seu diagnóstico (atributo) seja compartilhado com outro especialista (ator) sob um princípio de confidencialidade, mas ficaria chocado se o mesmo dado fosse vendido a uma empresa de marketing, pois isso violaria as normas contextuais.

\subsection{O Desafio do Capitalismo de Vigilância (Zuboff)}
\label{subsec:fund-zuboff}

Finalmente, a discussão sobre privacidade na era digital seria incompleta sem analisar a lógica econômica que a desafia em sua essência. Em sua obra \textit{A Era do Capitalismo de Vigilância}, a economista Shoshana Zuboff argumenta que a massiva coleta de dados por empresas de tecnologia não é um simples efeito colateral, mas a fundação de uma nova forma de mercado \cite{Zuboff2019}. Nesse modelo, as experiências humanas são tratadas como matéria-prima gratuita e processadas como "excedente comportamental". Este excedente é utilizado para fabricar "produtos de predição", que são vendidos em novos mercados com o objetivo de antecipar e até mesmo influenciar o comportamento dos usuários.

Essa lógica econômica choca-se frontalmente com as definições anteriores de privacidade: ela torna o ideal de controle individual de Westin uma ilusão, ao operar por meio da extração massiva e da ofuscação, e viola sistematicamente a integridade contextual de Nissenbaum, ao remover os dados de seu contexto original para alimentar outros mercados de futuros comportamentais. Com essa nova lógica mercadológica, a questão da privacidade se transforma de uma violação individual para um desafio sistêmico ao qual a legislação precisa se adaptar, começando por categorizar e definir precisamente o objeto de sua proteção.


\begin{figure}[H]
    \caption{A definição de privacidade ao longo do tempo}
    \label{fig:timeline-privacidade}
    \centering
    \includegraphics[width=\linewidth]{pictures/timeline-privacidade}
\fonte{Elaborado pelo autor}
\end{figure}


% --- NOVA ESTRUTURA - SEÇÃO 2: A LEI GERAL DE PROTEÇÃO DE DADOS (LGPD) ---
\section{A Lei Geral de Proteção de Dados Pessoais (LGPD)}
\label{sec:fund-lgpd}

A evolução do conceito de privacidade, especialmente a noção de "autodeterminação informativa" de Westin, constitui a base filosófica para os modernos marcos regulatórios de proteção de dados em todo o mundo. No Brasil, essa materialização se deu através da Lei Geral de Proteção de Dados Pessoais (LGPD), Lei nº 13.709/18. A criação da LGPD foi fortemente influenciada pelo Regulamento Geral de Proteção de Dados (GDPR) da União Europeia, que estabelece um nível adequado de proteção como requisito para a transferência internacional de dados, incentivando a harmonização legislativa global \cite{Doneda2019}.

O propósito da LGPD, explicitado em seu Art. 1º, transcende a noção clássica de privacidade como isolamento. O objetivo da lei é assegurar o direito à proteção dos dados pessoais e, ao mesmo tempo, tutelar outros direitos fundamentais de "liberdade e de privacidade e o livre desenvolvimento da personalidade da pessoa natural" \cite{Brasil2018lgpd}. Conforme aponta \citeonline{Doneda2019}, a proteção de dados adquire assim uma "dimensão coletiva", servindo como instrumento para coibir o uso de informações para fins discriminatórios contra indivíduos ou grupos.

Para alcançar seus objetivos, toda a lógica de tratamento de dados sob a LGPD é orientada por dez princípios fundamentais, listados em seu Art. 6º. Dentre eles, destacam-se o \textbf{princípio da finalidade}, que exige que o tratamento seja realizado para propósitos legítimos, específicos e informados ao titular; o \textbf{princípio da necessidade}, que limita o tratamento ao mínimo necessário para atingir suas finalidades; e o \textbf{princíio da transparência}, que garante aos titulares informações claras e acessíveis sobre o tratamento \cite{Brasil2018lgpd}. Esses princípios, segundo \citeonline{Doneda2019}, formam o núcleo de questões que qualquer ordenamento jurídico deve enfrentar ao regular a proteção de dados, servindo como a espinha dorsal para a interpretação e aplicação de toda a lei.

\subsection{Definições Fundamentais: Dado Pessoal e Dado Sensível}
...
Como consequência dessa distinção, entende-se que o dado pseudoanonimizado é identificável, e portanto continua sendo considerado um dado pessoal sob a LGPD cujo tratamento requer uma base legal, como o consentimento do titular. Já o dado anonimizado, por perder a possibilidade de associação a um indivíduo, deixa de ser considerado um dado pessoal e, portanto, pode ser tratado sem a necessidade de consentimento, desde que a anonimização seja efetiva e irreversível \cite{Brasil2018lgpd}. Isso significa que, uma vez anonimizado, o conjunto de dados pode ser utilizado para pesquisa sem as restrições impostas pela LGPD.

\begin{figure}[H]
    \caption{Pseudonimização vs Anonimização}
    \label{fig:diagrama-pseudoanonimizacao}
    \centering
    \includegraphics[width=\linewidth]{pictures/diagrama-pseudoanonimizacao}
\fonte{Elaborado pelo autor}
\end{figure}
...
O Art. 12 da LGPD\footnote{A LGPD (Lei nº 13.709/2018) foi parcialmente alterada pela Lei nº 13.853/2019, que criou a Autoridade Nacional de Proteção de Dados (ANPD) como autarquia de regime especial. As disposições materiais sobre anonimização não foram alteradas por essa emenda.} operacionaliza esse conceito ao estabelecer que os dados são considerados anonimizados quando não for razoavelmente possível identificar o titular \textit{"levando-se em conta os meios técnicos razoáveis e disponíveis na ocasião de seu tratamento"} \cite{Brasil2018lgpd} — formulação deliberadamente dinâmica e baseada em risco, não em inviolabilidade absoluta. A Autoridade Nacional de Proteção de Dados (ANPD), em seus Estudos Técnicos sobre Anonimização de 2023 \cite{ANPD2023juridica, ANPD2023computacional}, consolida essa interpretação: o que conta como anonimização efetiva é contextual, e k-anonimato e Privacidade Diferencial são explicitamente reconhecidos como técnicas computacionais admissíveis sob a LGPD \cite{ANPD2023computacional}. Em perspectiva de direito comparado, o Considerando 26 do GDPR europeu adota padrão estruturalmente análogo, condicionando o estatuto de dado anônimo à consideração de \textit{``todos os meios razoavelmente prováveis de serem utilizados''} para identificação \cite{GDPR2016} — convergência que reforça a legitimidade das abordagens aqui avaliadas em contextos internacionais de colaboração em pesquisa. Para os fins deste trabalho, essa perspectiva significa que reduzir o risco residual a níveis operacionalmente negligenciáveis — verificável empiricamente por auditorias de risco — constitui satisfação do requisito de anonimização do Art. 12 da LGPD.
...
Independentemente da estrutura do dado, seus atributos devem ser categorizados para a correta aplicação das técnicas de anonimização. Segundo \citeonline{Zuo2021Data} e \citeonline{Olatunji2024}, é prática consolidada classificar os atributos de risco em três categorias. . Primeiro, os Identificadores Diretos (ID), que explicitamente identificam um indivíduo, como o número do CPF ou uma combinação de nome e endereço. Em segundo lugar, os Quasi-Identificadores (QI), que são atributos que, isoladamente, não identificam ninguém, mas quando combinados podem levar à reidentificação com alta probabilidade, como gênero, data de nascimento e código postal \cite{Zuo2021Data, Olatunji2024}.
...
% ---
\subsection{Técnicas de Perturbação, Partição e Agregação}
...
Técnicas de \textbf{agregação e refinamento} também são amplamente utilizadas. A \textbf{microagregação}, por exemplo, é aplicada a atributos numéricos e funciona agrupando registros em pequenos clusters de tamanho mínimo $k$ e, em seguida, substituindo os valores de cada registro pelo centroide (média) do seu cluster \cite{Olatunji2024}. Já a \textbf{supressão}, frequentemente usada para evitar a generalização excessiva causada por outliers, pode ser aplicada em nível de registro (removendo a linha inteira, o que pode levar a uma perda excessiva de informação) ou em nível de célula (removendo apenas os valores problemáticos de QIs) \cite{Olatunji2024}. A supressão em nível de célula é geralmente preferível por preservar melhor a utilidade dos dados, tratando os valores suprimidos como dados faltantes, algo que muitos modelos de machine learning conseguem acomodar \cite{Olatunji2024}.
...
Um paradigma mais moderno e que oferece uma garantia de privacidade mais forte e matemática é a \textbf{Privacidade Diferencial (DP)}, formalizada por Dwork \cite{Dwork2006}. Diferente dos modelos anteriores que se focam em propriedades do dataset anonimizado, a DP é uma propriedade de um algoritmo de análise. A sua intuição fundamental é garantir que a inclusão ou exclusão de qualquer registro individual no conjunto de dados tenha um impacto estatisticamente insignificante no resultado de qualquer consulta ou análise realizada \cite{Olatunji2024}. Essa garantia robustece a proteção contra ataques de reconstrução e inferência.
...
\begin{figure}[H]
  \caption{O Balanço entre Privacidade e Utilidade na Anonimização}
  \label{fig:tradeoff_privacidade_utilidade}
  \centering
  \includegraphics[width=0.85\linewidth]{pictures/tradeoff_privacidade_utilidade}
\fonte{Elaborado pelo autor}
\end{figure}

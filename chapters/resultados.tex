...
A análise da \textit{Baseline} confirmou a vulnerabilidade interna dos dados: 99,98\% dos registros são únicos quando combinadas as variáveis Age, Gender e Ethnicity, configurando um risco máximo de Promotor de 100\% \textit{dentro do espaço dos quasi-identificadores declarados}. Cabe observar que o MIMIC-III é, em si, um banco de dados já submetido a de-identificação sob o padrão \textit{Safe Harbor} da HIPAA — datas de eventos foram deslocadas aleatoriamente por paciente, idades superiores a 89 anos foram uniformizadas em 91 anos, e identificadores diretos foram removidos \cite{Johnson2016}. O risco de 100\% expressa, portanto, a unicidade combinatorial de \{Age, Gender, Ethnicity\} \textit{dentro da coorte}, não a reidentificação em relação à população geral. Esta distinção é metodologicamente relevante: o experimento quantifica a redução de unicidade intra-coorte que cada técnica de pseudonimização proporciona, o que é a condição de privacidade relevante para contextos de compartilhamento institucional de dados clínicos. Esta condição inviabiliza, sob a ótica da LGPD, qualquer compartilhamento dos dados sem tratamento prévio para finalidades secundárias de pesquisa, mesmo entre instituições parceiras. A \autoref{tab:risco-reid} consolida o impacto dos três cenários de pseudonimização sobre essa condição.
...
Os três cenários alcançaram redução substancial do risco residual, levando a probabilidade máxima de reidentificação de 100\% para níveis abaixo de 4\% em todas as configurações testadas. O Cenário A, configuração sintática de proteção leve, reduziu o risco de Promotor a 3,57\% --- valor que corresponde à reidentificação correta de aproximadamente 1 a cada 28 registros sob o modelo de ataque mais permissivo, com supressão desprezível (apenas 2 registros eliminados como \textit{outliers}). Este valor é inferior ao limite superior teórico de $1/k = 20\%$ para $k=5$: o ARX determina a transformação ótima que satisfaz \textit{ao menos} $k=5$ como requisito mínimo, e as classes de equivalência resultantes — definidas pela interação entre as hierarquias de generalização e a distribuição dos dados — acabam sendo substancialmente maiores, com tamanho mínimo efetivo de 28 registros. O parâmetro $k$ é um requisito de piso, não um teto. O Cenário B, mediante a combinação de $k=10$ com $l$-diversidade ($l=2$) sobre o atributo \textit{Diagnosis}, reduziu o risco a 1,37\%, tornando estatisticamente improvável tanto a reidentificação direcionada quanto a inferência sobre o atributo sensível, preservando integralmente o volume amostral.
...
O Cenário B emerge como o ponto ótimo de equilíbrio identificado neste estudo. Sob esta configuração, o risco máximo de reidentificação reduz-se a 1,37\%, valor compatível com diretrizes operacionais consolidadas para dados de saúde \cite{ElEmam2013}. Do ponto de vista regulatório, este nível de risco residual pode ser interpretado como satisfação do requisito de anonimização do Art. 12 da LGPD, que condiciona a classificação de um dado como anonimizado à inviabilidade de reidentificação \textit{"levando-se em conta os meios técnicos razoáveis e disponíveis"}: com um Marketer risk de 0,04\% e Prosecutor risk de 1,37\%, a probabilidade de qualquer adversário identificar um titular específico usando os quasi-identificadores tratados é estatisticamente negligenciável \cite{ANPD2023juridica} — padrão análogo ao adotado pelo Considerando 26 do GDPR europeu, que condiciona o estatuto de dado anônimo à consideração de ``todos os meios razoavelmente prováveis de serem utilizados'' para reidentificação \cite{GDPR2016}. A utilidade clínica é preservada com degradação inferior a 0,5 ponto percentual de AUROC em ambas as arquiteturas testadas. Adicionalmente, o cenário preserva integralmente o volume amostral e mantém as disparidades de gênero próximas ao Baseline, configurando uma escolha defensável simultaneamente sob critérios de privacidade, utilidade e equidade.

O Cenário C atingiu o menor risco de Promotor (1,05\%), mas ao custo amostral de 31,1\%: 5.578 registros foram suprimidos pelo mecanismo ARX/SafePub para satisfazer o orçamento ($\epsilon=2{,}0$, $\delta=10^{-5}$). Este custo amostral tem implicação metodológica relevante: a degradação de utilidade observada no Cenário C confunde o efeito do mecanismo DP com o efeito de simplesmente treinar sobre menos dados — separar essas duas contribuições requeria um controle por subamostragem aleatória equivalente, não executado neste estudo. Cabe também precisar o escopo da garantia ($\epsilon, \delta$)-DP: ela aplica-se ao \textit{dataset} compartilhado, não ao modelo de \textit{Deep Learning} treinado sobre ele; a privacidade diferencial do modelo em si exigiria mecanismos de treinamento como DP-SGD. Este achado é coerente com comparações sistemáticas que documentam maior \textit{information loss} para Privacidade Diferencial em relação a combinações sintáticas como $k+l$ \cite{Fakeeroodeen2021, Pilgram2024}, sugerindo que o Cenário C deve ser preferido quando o orçamento amostral for folgado ou quando a preservação de Ethnicity em granularidade original for estritamente necessária — como em análises de equidade algorítmica estratificadas por etnia.
...
A perspectiva de gestão de risco adotada pela ANPD \cite{ANPD2023computacional} reforça que a exigência legal não é de inviolabilidade absoluta, mas de inviabilidade razoável de reidentificação no contexto do tratamento: o que importa é que os meios técnicos razoáveis disponíveis não permitam identificar o titular — e os resultados aqui reportados demonstram empiricamente que os Cenários B e C satisfazem esse critério. Estratégias de pseudonimização robusta, particularmente aquelas que combinam múltiplos critérios sintáticos como $k$-anonimato e $l$-diversidade, oferecem um caminho prático de \textit{compliance} com a LGPD compatível com a manutenção da inovação em IA aplicada à saúde \cite{ANPD2023juridica, GDPR2016}. O dilema regulatório de ``compartilhar tudo ou compartilhar nada'' é, portanto, um falso dilema: existe um espaço tecnicamente caracterizável e empiricamente defensável de configurações que conciliam privacidade efetiva e utilidade clínica.
